...
- [ ] [SRS-03/AC-01] `cargo test --lib --no-default-features` passes for still image and audio probing paths without ffprobe available <!-- verify: cargo test, SRS-03:start:end -->
- [ ] [SRS-03/AC-02] Video-like inputs probed without the `video` feature return `MediaKind::Unknown` rather than panicking or attempting to spawn ffprobe <!-- verify: cargo test, SRS-03:start:end -->
