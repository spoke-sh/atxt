...
#### Summary
Expose the timed summary pipeline through the CLI and publish the canonical mission proof so operators can inspect the same timed fixture through direct and degraded terminal paths from `just screen`.
...
- [x] [SRS-NFR-01/AC-03] `just screen` exposes the timed-sequence operator signal from the primary human entrypoint without introducing sequence-local terminal heuristics <!-- verify: cd /home/alex/workspace/spoke-sh/atext && just screen >/dev/null, SRS-NFR-01:start:end, proof: ac-3.log-->
